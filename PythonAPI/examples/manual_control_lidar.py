...
class CameraManager(object):
    def __init__(self, parent_actor, hud, gamma_correction):
        ...
        self.sensors = [
            ...
            ['sensor.lidar.ray_cast', None, 'Lidar (Ray-Cast)', {'range': '100'}], #
    ...
    def save_lidar_data(self):
        ...
        coords_dir = os.path.join(save_dir, 'coords', f"{file_name}.txt")

        # Saving 
        if self.lidar_output_data is not None:
    ...
    def _parse_image(weak_self, image):
        # print(f"Parse image in thread: {threading.current_thread().name}")
        self = weak_self()
        if not self:
            return
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            self.lidar_output_data = np.array(points)
